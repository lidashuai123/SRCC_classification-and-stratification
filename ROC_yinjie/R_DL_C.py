...
def get_roc_and_meters(values, curves_name, save_path, filename, threshold=0.5):
    ...
    for idx, (targets, scores) in enumerate(values):
        ...
        plt.title('Receiver operating characteristic curves (Training cohort)', fontsize=17)
...
# ####五个模型的结果
# # input_values = [[train_label, R_train_cohort_pro], [train_label, DL_train_cohort_pro],
# #                 [train_label, Unet_train_cohort_pro], [train_label, Clinical_train_cohort_pro],
# #                 [train_label, Merge_train_cohort_pro]]
# input_values = [[test_label, R_test_cohort_pro], [test_label, DL_test_cohort_pro],
#                 [test_label, Unet_test_cohort_pro], [test_label, Clinical_test_cohort_pro],
#                 [test_label, Merge_test_cohort_pro]]
# input_names = ['R model', 'DL model', 'Unet encoder model', 'Clinical model', 'AI model']

####四个模型的结果
input_values = [[train_label, R_train_cohort_pro],
                [train_label, Unet_train_cohort_pro], [train_label, Clinical_train_cohort_pro],
                [train_label, Merge_train_cohort_pro]]
# input_values = [[test_label, R_test_cohort_pro],
#                 [test_label, Unet_test_cohort_pro], [test_label, Clinical_test_cohort_pro],
#                 [test_label, Merge_test_cohort_pro]]
input_names = ['R model', 'DL model', 'Clinical model', 'AI model']

save_path = r'E:\Radiomics\huaxi_jiang_yinjie\outcome\graph\performance'
if not os.path.exists(save_path):
    os.makedirs(save_path)
file_name = 'Training0810.pdf'
get_roc_and_meters(input_values, input_names, save_path, file_name, threshold=0.5)



# dict_info_train = {'train_cohort_label': train_cohort_label, 'train_cohort_pro': train_cohort_pro}
# dict_info_test = {'test_cohort_label': test_cohort_label, 'test_cohort_pro': test_cohort_pro}
# info_path_train = pd.DataFrame(dict_info_train)
# info_path_test = pd.DataFrame(dict_info_test)age.py
#
# file_path_save_path_train = os.path.join(r'E:\Radiomics\Lung_Grading\outcome\stratify_sheet', 'all_train.xlsx')
# writer_train = pd.ExcelWriter(file_path_save_path_train)
# info_path_train.to_excel(writer_train)
# writer_train.save()
#
# file_path_save_path_test = os.path.join(r'E:\Radiomics\Lung_Grading\outcome\stratify_sheet', 'all_test.xlsx')
