...
print(age_median)
for i, j, p1 in zip(age, label, Merge):
    if i <= age_median:
        train_num += 1
...
file_path_save_path_train = os.path.join(r'E:\Radiomics\huaxi_jiang_yinjie\outcome\graph\stratified_sheet', 'age_train.xlsx')
writer_train = pd.ExcelWriter(file_path_save_path_train)
info_path_train.to_excel(writer_train)
writer_train.save()

file_path_save_path_test = os.path.join(r'E:\Radiomics\huaxi_jiang_yinjie\outcome\graph\stratified_sheet', 'age_test.xlsx')
