...
pretrained_weight_path = r'E:\pycharm_project\Huaxi_Yinjie_Jiang\AP_weights\epoch_9_712_704.pth'##PVP
...
def valid(epoch):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (img, label) in enumerate(validloader):
            ...
            label_array = label.cpu().numpy()
            all_label = np.hstack([all_label, label_array])
            # auc = roc_auc_score(label_array, out_auc)
            loss = criterion(out, label)
            # print("Valida__Epoch:%d [%d|%d] loss:%f， auc:%f" % (epoch, batch_idx, len(trainloader), loss.item(), auc))
        auc = roc_auc_score(all_label, all_out)

        print("Valid__Epoch:%d [%d|%d] loss:%f， auc:%f" % (epoch, batch_idx, len(trainloader), loss.item(), auc))
...
if __name__ == '__main__':
    ...
    global_step = 0
    metrics = []
    save_path = r'E:\pycharm_project\Huaxi_Yinjie_Jiang\AP_weights'
